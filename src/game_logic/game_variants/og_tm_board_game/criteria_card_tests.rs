...
use crate::game_logic::{SmallIndex, SingleEval, Card, Code, Machine};
#[rustfmt::skip]
pub fn populate_machine_feedback_2(
    code: Code, 
    machine: &mut Machine
) -> crate::game_logic::TuringCodeEval {
    ...
    macro_rules! push_eval {
        ($idx:expr, $($cond:expr),+) => {
            if !machine.contains_key(&Card($idx)) { 
                machine.insert(
                    Card($idx), 
                    {
                ...
                machine.get_mut(&Card($idx)).unwrap().insert(
                    code.clone(),
                    vec![$($cond),+],
                );
            }
