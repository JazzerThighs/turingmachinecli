...
fn main() {
    ...
    let (min_code, max_code, min_digit, max_digit, mode, difficulty, test_amount, og_tm_game) = setup::set_game_parameters();
    ...
    let puzzle: Puzzle =
        generate_puzzle(&matrix, &mode, &difficulty, test_amount, target_code, og_tm_game);
    for section in puzzle.tests.iter() {
        for test in section.iter() {
            println!("Test: {}, Card: {}", test, &matrix[0].checks[test.clone()].0);
        }
    }
}
